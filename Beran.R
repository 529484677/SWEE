...
NWweight2 = function(Z, Z.sample, hn, lam=rep(1,n)){
  ...
  for(i in 1:p1){
    ...
    if(sum(dMZ[,,i]==1|dMZ[,,i]==-1|dMZ[,,i]==0)==(n*n) & any(dMZ[,,i]!=0)){
      Kp[,,i]=KK2(dMZ[,,i],hn[i]) # if Z is a binay covariate
    }
    else{
      Kp[,,i]=KK(dMZ[,,i],hn[i]) # if Z is a continous covariate
    }
...
P.T = function(data, w){
  ## compute P(T=inifint|Z.j) = product_i {1 - b.ni(x)/sum_k B.nk(x)I(Y.k>=t)}^{I(delta.i=1)}, at t=infinit=Y.n, Z=Z.j, j=1,...,n
  ## namely S.T(t|x) at t>=Y.(n)
  #
  # data:
  # w: NWeight = B.nk(x), dim is n*k, k =1,...,n
  # output: P(T=inifint|Z.j), dim is j*1 , j=1,...,n
  ...
  q = t(apply(q, 1, prod))  # P(T=infinit|Z.j) = S(Y.(n)|Z.j) = product_i {1 - B.ni(x)/ sum_k B.nk(x)*I(Y.(k)>=t)} at Z=Z.j, dim is n*1, the j row is the j person at S(Y.(n)|Z.j)
...
S.T = function(t, covrat, covrat.sample, h.inr, data){
  ## Beran estimator, compute S.T(t|Z.j), dim is m*1, Z.j denotes the j-th person, j=1,...,m
  # where S.T(t|x) = product_i {1 - H1(ds|x)/(H([Y.i,infinit)|x)-P(T=infinit|x)*F.C([Yi,infinit)|x}^I(Y.i<=t,delta=1) 
  #
  # t: dim is 1
  # Z: the Argument Z.j in S.T0(t|Z.j), Z could be 1*p (1 person) or m*p (m persons), also is the Argument Z in B_nk(Z)
  # Z.sample: dim is n*p, the n observations of Z 
  # h.inr: dim is p*1, the bandwidth
  #
  # output: 
  # S(t|Z.j) , dim is m*1, j=1,...,m, t is 1*1


  w = NWweight2(Z = covrat, Z.sample = covrat.sample, hn = h.inr, lam=lam)   # B.nk(z), dim is j*k, j=1,...m (m persons in z), k=1,...,n of n observaions in Z

  
  # S.T(Y.i|Z.j)  dim is j*i, j=1,...,m, i=1,...,n
  m = nrow(w)
  n = ncol(w)
  if(m>1){k= cbind(rep(0,m),w[,1:(n-1)]) # add 0 at first col and delete the last col of B.nk(x)
  }else{k=t(c(rep(0,m), w[,1:(n-1)]))  }
  # k = cbind(rep(0,m), w[,1:(n-1)])  # add 0 at first col and delete the last col of B.nk(x)
  cumsum.k = t(apply(k, 1, cumsum))  # sum_i B.ni(x)*I(Y.(i)<=t), the 1 col is t=Y.(0)=0, the 2 col is t=Y.(1), ...., the n col is t=Y.(n-1) 
  cum.w = (1 - cumsum.k)   # H([t,infinit)|x), the j roe is the j person, the k col is this value H at t=Y.(k)   
  ...
  Y.i = data$Y  # Y.(1),...,Y.(n)
  t.indc = ifelse(Y.i<=t,1,0)  # I(Y.i<=t)
  t.matr = matrix(t.indc, nrow = m, ncol = n, byrow = TRUE)
  q = replace(q, t.matr==0, 1)  # if I(Y.i<=t)=0, then q=1
  f = apply(q, 1, prod)  # S.T0(t|Z.j), dim is m*1
  return(as.numeric(f))
}
...
NWweight2.largedata = function(Z, Z.sample, hn){
  ## calculate the Nadaraya-Watson weights B_nk(Z) of dim m*k, Z is m*p, k=1,..,n of n observations of Z
  #
  # hn: bandwitdth dim is p*1
  # Z: the Argument Z in  B_nk(Z), Z could be 1*p (1 person) or m*p (m persons)
  ...
  # Z.sample = as.data.frame(Z.sample); rownames(Z.sample)= paste("Obser",1:n,sep=""); colnames(Z.sample)=paste("Z_",1:p1,sep="")
  # Z = as.data.frame(Z)
  # Z.sample = as.data.frame(Z.sample);

  
  fun = function(x){
    ## get K(x-Z.k), dim is 1*n, k=1,...,n, where x is the Z of the m-th person
    # x is p*1
    # Z.k = Z.sample, dim is n*p
    
    # for Z1
    temp1 = (x-t(Z.sample))[1,]  # dim is p*n, x - Z.k, k=1,...,n, where x is the Z of the m-th person, p is 2
    if(Z.status[1]==1){
      temp1 = KK(temp1,hn[1])  # K(z-Z.k / hn) for continuous variable
    }else{
      temp1 = KK2(temp1,hn[1])  # K(z-Z.k / hn) for discrete variable
    }
    
    # for Z2
    temp2 = (x-t(Z.sample))[2,]  # dim is p*n, x - Z.k, k=1,...,n, where x is the Z of the m-th person
    if(Z.status[2]==1){
      temp2 = KK(temp2, hn[2])  # K(z-Z.k / hn) for continuous variable
    }else{
      temp2 = KK2(temp2,hn[2])  # K(z-Z.k / hn) for discrete variable
    }
    f = temp1*temp2  # K(z-z.k) = K(z1-z1.k) * K(z2-z2.k)
    remove(temp1, temp2)
    return(f)
  }
  
  prodKp = t(apply(Z, 1, fun))  # dim is m*n, K(x) = K(Z.1)*...* K(Z.p), since Z is p dims. K(x) is m*n.
  B_nk = prodKp / apply(prodKp, 1, sum)
  return(B_nk)
}
