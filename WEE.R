...
Get.Data = function(){
  ...
  Get.random <- function() {
    ...
    for (i in 1:n) {
      ...
      S0 =  function(t){
        f = exp(-H_0(t) * exp(Z[i,]%*%beta.true))
      } 
      T0[i] <- uniroot(function(t) {1 - S0(t) - u[i]}, c(0, 100))$root
    }
    ...
    data = data.frame(Y=Y, delta=delta, Z=Z)
...
Repeat = function(r){
  ...
  ##  max the weighted partial likelihood function and using Newton-Raphson algorithm to get beta.estimate 
  loglk = function(beta){
  ...
  myfit = maxLik(loglk, start = rep(0, length(beta.true)), method = 'NR')  # Newton-Raphson algorithm
  beta.our = myfit$estimate
  
  # standard error using purterbation bootstrap method 
...
packagelist = c( 'maxLik')
Data = Get.Data()  # the simulated data over 800 repetations 
